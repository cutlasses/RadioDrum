...
class VALIDATE_STREAM_EFFECT : public AudioStream
{
...
public:
  ...
  virtual void        update() override
  {
      ...
      if( in_block == nullptr )
      {
        DEBUG_TEXT("Unable to receive audio ");
        DEBUG_TEXT_LINE(m_id);
      }
      else
      {
        //DEBUG_TEXT("ABLE to receive audio ");
        //DEBUG_TEXT_LINE(m_id);
        transmit(in_block, 0);
        release(in_block);
      }
...
namespace DSP_UTILS
{
  ...
  inline void fade_in_linear( int16_t* sample_buffer, int buffer_size )
  {
    ...
    for( int i = 0; i < buffer_size; ++i )
    {
      ...
      sample = round_to_int( fade * sample );
      fade += fade_inc;
    }
  }
